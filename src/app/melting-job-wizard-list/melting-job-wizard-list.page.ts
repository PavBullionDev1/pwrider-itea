...
@Component({
  ...
  templateUrl: './melting-job-wizard-list.page.html',
  styleUrls: ['./melting-job-wizard-list.page.scss'],
	providers: [NativeAdapterService],
})
export class MeltingJobWizardListPage implements OnInit {
  ...
  getAllCustomer() {
	...
	getSupplierList() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetMeltingJobList/" + localStorage['token'] + "?is_wizard=true"
			)
			.subscribe(
				(data: any) => {
