...
@Component({
  ...
  templateUrl: './melting-job-list.page.html',
  styleUrls: ['./melting-job-list.page.scss'],
	providers: [NativeAdapterService],
})
export class MeltingJobListPage implements OnInit {
  ...
  setFilter(filter: string) {
    ...
    if (filter === 'inProgress') {
      ...
      this.filteredList = this.dataList.filter(item => item.status == 0);
	...
	getSupplierList() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetMeltingJobList/" + localStorage['token']
			)
			.subscribe(
				(data: any) => {
