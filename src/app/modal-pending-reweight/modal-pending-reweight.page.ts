...
@Component({
  selector: 'app-modal-pending-reweight',
  templateUrl: './modal-pending-reweight.page.html',
  styleUrls: ['./modal-pending-reweight.page.scss'],
})
export class ModalPendingReweightPage implements OnInit {
  ...
  camera_mode = 0;
	...
	async ngOnInit() {
		...
		this.grnIndex = index;
		console.log("ngOnInit", this.grnIndex);
    ...
    if (selectedItem.gram != 0 && selectedItem.gram != '' && selectedItem.gram != null) {
      selectedItem.gram = Big(selectedItem.gram).round(2, 0).toNumber();
		...
		console.log("selectedPresetPurity", selectedPresetPurity);

		if(selectedPresetPurity.length > 0) {

			this.modalData.is_editable = selectedPresetPurity[0].is_editable;
			let final_percent = selectedPresetPurity[0].override_gold_pure_percent;
			...
			//console.log("final_percent", final_percent);
	...
	async openCamera(sourceType: CameraSource) {
		...
		try {
			...
			// StatusBar调用已移除
			// await CapacitorStatusBar.setOverlaysWebView({ overlay: true });
			...
			try {
				...
				if (capturedPhotos.photos.length > 0) {
					...
					for (const photo of capturedPhotos.photos) {
						...
						const base64String = await ModalPendingReweightPage.convertBlobToBase64(blob) as string;
