...
@Component({
  ...
  templateUrl: './assay-report-clock-in-list.page.html',
  styleUrls: ['./assay-report-clock-in-list.page.scss'],
	providers: [NativeAdapterService],
})
export class AssayReportClockInListPage implements OnInit {
  ...
  getAllCustomer() {
	...
	getSupplierList() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetAssayReportList/" + localStorage['token']
			)
			.subscribe(
				(data: any) => {
