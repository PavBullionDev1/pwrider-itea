...
export class TestDetailPage implements OnInit {
  ...
  saveDraft() {
    ...
    localStorage.setItem(`testDetailDraft_${this.goods_received_note_id}`, JSON.stringify(draft));
    console.log('Save Draft');
  }

  loadDraft() {
    const draftString = localStorage.getItem(`testDetailDraft_${this.goods_received_note_id}`);
  ...
  get_grn_data() {
		...
		this.httpClient
			.get(
				this.App.api_url + "/appGetPendingTestGrnData/" + this.goods_received_note_id + "/" + localStorage['token'])
			.subscribe(
				(data: any) => {
          ...
          const savedDraft = localStorage.getItem(`testDetailDraft_${this.goods_received_note_id}`);
          if(!savedDraft){
	...
	calculate_summary() {
		...
		this.formData.detailList.forEach(function (v: any, k: number) {
          if(v.gram > 0){
	...
	async one_image_upload(tobeSubmit: any) {
		try {
			const data: any = await this.httpClient.post(this.App.api_url + "/uploadImage", tobeSubmit, { observe: "body" }).toPromise();
			if (data['status'] === "OK") {
				this.App.stopLoading(100);
	...
	async multiple_image_upload(tobeSubmit: any) {
		try {
			const data: any = await this.httpClient.post(this.App.api_url + "/uploadMultipleImage", tobeSubmit, { observe: "body" }).toPromise();
			if (data['status'] === "OK") {
				this.App.stopLoading(100);
	...
	submit_now() {
		...
		this.httpClient.post(this.App.api_url + "/appSubmitPendingTestGrnData", tobeSubmit, { observe: "body" }).subscribe((data: any) => {
			...
			if (data.status == "OK") {
				...
				//remove draft
				localStorage.removeItem(`testDetailDraft_${this.goods_received_note_id}`);
