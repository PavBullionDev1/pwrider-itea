...
@Component({
  selector: 'app-bullion-pending-tbc',
  templateUrl: './bullion-pending-tbc.page.html',
  styleUrls: ['./bullion-pending-tbc.page.scss'],
	providers: [],
})
export class BullionPendingTBCPage implements OnInit {
  ...
  getAllCustomer() {
	...
	get_result_list() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetPendingTBCList/" + localStorage['token'] + "?customer_id=" + this.selectedCustomer
			)
			.subscribe(
				(data: any) => {
					...
					this.resultList = Object.values(this.resultList);
