...
@Component({
  ...
  templateUrl: './modal-gold-shipment-detail.page.html',
  styleUrls: ['./modal-gold-shipment-detail.page.scss'],
})
export class ModalGoldShipmentDetailPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.out_stock_image = this.items['out_stock_image'] ? this.items['out_stock_image'] : [];
  ...
  submit() {
    ...
    this.items['out_stock_image'] = this.out_stock_image ? this.out_stock_image : [];
  ...
  async one_image_upload(tobeSubmit: any) {
    try {
      ...
      if (data['status'] === "OK") {
        this.isLoading = false;
        this.out_stock_image.push(data['result']['img_path']);
  ...
  // Delete item method (alias for delete_img)
  deleteItem(item: any) {
    this.delete_img(item);
  }
