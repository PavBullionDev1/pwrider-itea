...
@Component({
  selector: 'app-modal-qc',
  templateUrl: './modal-qc.page.html',
  styleUrls: ['./modal-qc.page.scss'],
})
export class ModalQcPage implements OnInit {
			...
			try {
				...
				if (capturedPhotos.photos.length > 0) {
					...
					for (const photo of capturedPhotos.photos) {
						...
						const base64String = await ModalQcPage.convertBlobToBase64(blob) as string;
