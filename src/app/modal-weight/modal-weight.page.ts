...
@Component({
  selector: 'app-modal-weight',
  templateUrl: './modal-weight.page.html',
  styleUrls: ['./modal-weight.page.scss'],
})
export class ModalWeightPage implements OnInit {
	...
	async deleteItem(selectedItem){
      ...
      if (selectedItem.gram != 0 && selectedItem.gram != '' && selectedItem.gram != null) {
		...
		if(selectedPresetPurity.length > 0) {
			...
			selectedItem.item_code = selectedPresetPurity[0].gold_pure_id;
			...
			try {
				...
				if (capturedPhotos.photos.length > 0) {
					...
					for (const photo of capturedPhotos.photos) {
						...
						const base64String = await ModalWeightPage.convertBlobToBase64(blob) as string;
