...
@Component({
	selector: 'app-bullion-pending-check',
	templateUrl: './bullion-pending-check.page.html',
	styleUrls: ['./bullion-pending-check.page.scss'],
	providers: [],
})
export class BullionPendingCheckPage implements OnInit {
  ...
  getAllCustomer() {
	...
	get_result_list() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetPendingCheckList/" + localStorage['token'] + "?customer_id=" + this.selectedCustomer
			)
			.subscribe(
				(data: any) => {
