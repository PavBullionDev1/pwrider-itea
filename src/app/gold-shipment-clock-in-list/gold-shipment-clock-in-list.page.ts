...
@Component({
  ...
  templateUrl: './gold-shipment-clock-in-list.page.html',
  styleUrls: ['./gold-shipment-clock-in-list.page.scss'],
	providers: [NativeAdapterService],
})
export class GoldShipmentClockInListPage implements OnInit {
  ...
  getAllCustomer() {
	...
	getSupplierList() {
		...
		this.HttpClient
			.get(
				this.App.api_url + "/appGetClockInList/" + localStorage['token']
			)
			.subscribe(
				(data: any) => {
