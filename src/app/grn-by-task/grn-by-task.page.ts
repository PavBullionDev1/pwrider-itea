...
@Component({
	selector: 'app-grn',
	templateUrl: './grn-by-task.page.html',
	styleUrls: ['./grn-by-task.page.scss'],
})
export class GrnByTaskPage implements OnInit {
  ...
  saveDraft() {
    ...
    localStorage.setItem(`grnDraft_${this.task_id}`, JSON.stringify(draft));
    console.log('draft saved', localStorage.getItem(`grnDraft_${this.task_id}`));
    // this.App.presentAlert_default('Draft Saved', 'Your draft has been saved successfully.', "assets/gg-icon/attention-icon.svg");
  }

  loadDraft() {
    console.log('loadDraft',localStorage.getItem(`grnDraft_${this.task_id}`));
    const draft = JSON.parse(localStorage.getItem(`grnDraft_${this.task_id}`));
    if (draft) {
      this.grnList = draft.grnList || [];
      this.remark = draft.remark || '';
      this.customer_remark = draft.customer_remark || '';
      this.rounding_xau = draft.rounding_xau || '';
      this.purchase_order_id = draft.purchase_order_id || '';
      this.is_grn = draft.is_grn || false;
      this.is_tng = draft.is_tng || false;
      this.imageUrl_tng = draft.imageUrl_tng || '';
      this.MultipleImageUrl_tng = draft.MultipleImageUrl_tng || [];
      this.is_multiple_image_tng = draft.is_multiple_image_tng || false;
      this.MultipleImageUrl_DO = draft.MultipleImageUrl_DO || [];
      this.geolocation_lat = draft.geolocation_lat || '';
      this.geolocation_lng = draft.geolocation_lng || '';
      this.geolocation_photo = draft.geolocation_photo || '';
      this.estimated_XAU = draft.estimated_XAU || 0;
      this.update_po_item = draft.update_po_item || false;
      this.payCash = draft.payCash || false;
      this.warehouseList = draft.warehouseList || [];
      this.thisRoundShouldCollectXAUAmount = draft.thisRoundShouldCollectXAUAmount || 0;
      this.payment = draft.payment || { signature: '', amount: 0, remark: '', image: '' };
      console.log('Load Draft',this.grnList);
    }else{
      this.first_open = 1;
    }
  }
  ...
  getCollectionDetails() {
		...
		this.httpClient
			.get(
				this.App.api_url + "/appGetSupplierDetailByTask/" + this.task_id + "/" + localStorage['token'])
			.subscribe(
				(data: any) => {
          ...
          this.purchase_order_id = data.result['purchase_order_id'];
					this.customer_id = this.customerList['customer_id'];
          this.customer_name = this.customerList['name'];

          const savedDraft = localStorage.getItem(`grnDraft_${this.task_id}`);
          if(!savedDraft){
  ...
  handleCheckboxChange(checkboxType: 'is_tng' | 'is_grn') {
    ...
    tmp_XAU_amount = Big(tmp_XAU_amount).minus(total_imported_XAU).round(3, 0).toNumber();

    tmp_total_gram = Big(tmp_total_gram).round(3, 0).toNumber();
	...
	normal_submit_grn() {
      ...
      'total_XAU': this.priceUpdate()['total_XAU_amount']
		}
	...
	postData(tobeSubmit) {
        ...
        //remove draft
        localStorage.removeItem(`grnDraft_${this.task_id}`);
	...
	async openCamera_tng(sourceType: CameraSource) {
		try {
			console.log('openCamera_tng: 开始拍照，sourceType:', sourceType);
			...
			const capturedPhoto = await Camera.getPhoto(options);
			console.log('openCamera_tng: 拍照成功，开始loading');
			this.App.presentLoading(100000);
			...
			console.error('openCamera_tng: 拍照失败', error);
			this.App.stopLoading(100);
			await this.presentErrorAlert('Error', 'Failed to capture photo: ' + error.message);
		}
	}
	...
	async one_image_upload_tng(tobeSubmit: any) {
		try {
			console.log('one_image_upload_tng: 开始上传TNG图片');
			const data = await this.httpClient.post(this.App.api_url + "/uploadImage", tobeSubmit, { observe: "body" }).toPromise();
			
			if (data['status'] === "OK") {
				console.log('one_image_upload_tng: 上传成功');
				this.App.stopLoading(100);
			...
			console.error('one_image_upload_tng: 上传失败', error);
			this.App.stopLoading(100);
			await this.presentErrorAlert('Error', 'Failed to upload image');
		}
	}
	...
	async multiple_image_upload_tng(tobeSubmit: any) {
		try {
			console.log('multiple_image_upload_tng: 开始上传多张TNG图片');
			const data = await this.httpClient.post(this.App.api_url + "/uploadMultipleImage", tobeSubmit, { observe: "body" }).toPromise();
			
			if (data['status'] === "OK") {
				console.log('multiple_image_upload_tng: 上传成功');
				this.App.stopLoading(100);
				this.is_multiple_image_tng = 1;
			...
			console.error('multiple_image_upload_tng: 上传失败', error);
			this.App.stopLoading(100);
			await this.presentErrorAlert('Error', 'Failed to upload images');
		}
	}

	async delete_img(img_path: string, target_index: number) {
		const alert = await this.alertController.create({
			header: 'Confirmation',
			...
			buttons: [
				...
				},
				{
					text: 'Ok',
					handler: () => {
						const index = this.grnList[target_index].MultipleImageUrl.indexOf(img_path);
						if (index !== -1) {
							this.grnList[target_index].MultipleImageUrl.splice(index, 1);
						}
						if (this.grnList[target_index].showImage === img_path) {
							this.grnList[target_index].showImage = '';
						}
						this.haveItemImages = false;
						for (const item of this.grnList) {
							if (item.MultipleImageUrl.length > 0) {
								this.haveItemImages = true;
								break;
							}
						}
	...
	async delete_img_tng(img_path: string, type: number) {
		const alert = await this.alertController.create({
			header: 'Confirmation',
			message: 'Do you want to remove this photo?',
			buttons: [
				...
				text: 'Ok',
					handler: () => {
						if (type === 0) {
							const index = this.MultipleImageUrl_tng.indexOf(img_path);
						if (index !== -1) {
							...
							const index = this.MultipleImageUrl_DO.indexOf(img_path);
						if (index !== -1) {
							this.MultipleImageUrl_DO.splice(index, 1);
						}
							if (this.ShowImageUrl_do === img_path) {
							this.ShowImageUrl_do = '';
						}
						} else if (type === 2) {
						this.payment.image = '';
					}
	...
	async openModalSign() {
		...
		modal.onDidDismiss().then((data => {
			...
			// console.log("Signature data", this.payment.signature);
		}));
